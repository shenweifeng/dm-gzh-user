	...
	return chart = new Highcharts.Chart({
		...
		tooltip: { //鼠标滑向数据区显示的提示框
			formatter: function() {  //格式化提示框信息
				if(chartBarDatas.unit){
					return '<b>' + this.series.name + '</b>(' + this.x + '): ' + this.y + '(' + chartBarDatas.unit + ')';
				}
...
function createChartPie(chartPieDatas){
    return chart = new Highcharts.Chart({
        chart: {
          ...
          tooltip: {
			  formatter: function() {
				  if(chartPieDatas.unit){
					  return '<b>' + this.point.name + '</b>: ' + this.y + '(' + chartPieDatas.unit + ')';
				  }
...
/* 居中显示提示信息 */
tips_on_center = function(_msg){
	$.messager.show({
		title:'系统提示',
		msg: "<span style='color: red;'>"+_msg+"</span>",
		timeout: 3000,
		showType:'fade',
		// height: 100,
		style:{ top: $(window).height()/2 - 50 }
	});
}
